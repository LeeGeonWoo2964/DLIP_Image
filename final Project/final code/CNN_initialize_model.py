...
def initialize_model(model_name, num_classes, feature_extract, use_pretrained=True):
    # model_name: 사용할 모델명 
    # num_classes: 츨력층 노드(class의 수)
    # 전체를 학습할지 출력층만 학습할지 설정
    # use_pretrained: ImageNet에서 학습된 가중치 사용 여부부
    ...
    if model_name == "resnet":
        ...
        model_ft = models.vgg11_bn(pretrained=use_pretrained)
            # ImageNet으로 학습된 VGG11 모델을 불러옴
        set_parameter_requires_grad(model_ft, feature_extract)
            # 출력층을 제외한 나머지 레이어 가중치 고정
        num_ftrs = model_ft.classifier[6].in_features
            # in_features: 이 레이어가 받는 차원의 수 (입력 노드 수)
            # [6]: Linear, ReLu, Pooling, Dropout을 포함한 모든 층 중 마지막의 index
        model_ft.classifier[6] = nn.Linear(num_ftrs,num_classes)
            # 방금 받아온 입력 노드 수를 받아와 마지막 출력 노드 수를 고치는 코드 
        input_size = 224
